        ...
        }
        {
            ...
            for (i, entry) in self.audit_log.iter().enumerate() {
                let json = serde_json::to_string(entry)
                    .map_err(|_| EdisonError::SaveFailed)?;
                table.insert(i.to_string().as_str(), json.as_str())
                    .map_err(|_| EdisonError::SaveFailed)?;
            }
        }
        write_txn.commit()
...
pub fn encrypt_payload(
    ...
    let payload = Payload { msg: data, aad: aad.as_bytes() };
...
pub fn decrypt_payload(
    ...
    let cipher = Aes256Gcm::new(key);
    let nonce = Nonce::from_slice(nonce_bytes);
    let payload = Payload { msg: encrypted, aad: aad.as_bytes() };
    cipher
...
mod tests {
    ...
    fn owner_can_read_critical() {
        let r = Record::new("rec:1", DataTier::Critical,
    ...
    fn non_owner_cannot_read_critical() {
        let r = Record::new("rec:2", DataTier::Critical,
            "owner_abc", vec![1,2,3], [0u8; 32]).unwrap();
    ...
    fn admin_cannot_read_critical() {
        let r = Record::new("rec:3", DataTier::Critical,
            "owner_abc", vec![1,2,3], [0u8; 32]).unwrap();
    ...
    fn noise_readable_by_anyone() {
        let r = Record::new("rec:4", DataTier::Noise,
    ...
    fn record_without_owner_rejected() {
        let result = Record::new("rec:5", DataTier::Personal,
            "", vec![1], [0u8; 32]);
    ...
    fn record_has_timestamp() {
        let r = Record::new("rec:6", DataTier::Personal,
    ...
    fn owner_can_read_stored_record() {
        ...
        let r = Record::new("rec:10", DataTier::Personal,
            "owner_abc", vec![1,2,3], [0u8; 32]).unwrap();
    ...
    fn attacker_cannot_read_stored_record() {
        let mut store = Store::new();
        let r = Record::new("rec:11", DataTier::Critical,
            "owner_abc", vec![1,2,3], [0u8; 32]).unwrap();
        store.write(r).unwrap();
        assert_eq!(
    ...
    fn write_creates_audit_entry() {
        let mut store = Store::new();
        let r = Record::new("rec:20", DataTier::Personal,
    ...
    fn multiple_writes_all_audited() {
        ...
        for i in 0..5 {
            let id = format!("rec:{}", i);
            let r = Record::new(&id, DataTier::Noise,
                "owner_abc", vec![], [0u8; 32]).unwrap();
            store.write(r).unwrap();
        }
    ...
    fn granted_read_is_audited() {
        let mut store = Store::new();
        let r = Record::new("rec:30", DataTier::Personal,
            "owner_abc", vec![1], [0u8; 32]).unwrap();
        store.write(r).unwrap();
    ...
    fn denied_read_is_audited() {
        let mut store = Store::new();
        let r = Record::new("rec:31", DataTier::Critical,
            "owner_abc", vec![1], [0u8; 32]).unwrap();
        store.write(r).unwrap();
    ...
    fn list_returns_owner_records_only() {
        let mut store = Store::new();
        let r1 = Record::new("rec:60", DataTier::Personal,
            "alice", vec![1], [0u8; 32]).unwrap();
        let r2 = Record::new("rec:61", DataTier::Noise,
    ...
    fn owner_can_delete_own_record() {
        let mut store = Store::new();
        let r = Record::new("rec:70", DataTier::Personal,
            "alice", vec![1], [0u8; 32]).unwrap();
        store.write(r).unwrap();
    ...
    fn non_owner_cannot_delete_record() {
        let mut store = Store::new();
        let r = Record::new("rec:71", DataTier::Critical,
            "alice", vec![1], [0u8; 32]).unwrap();
        store.write(r).unwrap();
        assert_eq!(
    ...
    fn payload_encrypts_and_decrypts() {
        ...
        let encrypted = encrypt_payload(
            original, &key, "rec:crypto", &DataTier::Personal
        ).unwrap();
        assert_ne!(encrypted, original.to_vec());
        let decrypted = decrypt_payload(
            &encrypted, &key, "rec:crypto", &DataTier::Personal
        ).unwrap();
    ...
    fn aad_mismatch_fails_decryption() {
        ...
        let original = b"sovereign data";
        let encrypted = encrypt_payload(
            original, &key, "rec:aad-test", &DataTier::Critical
        ).unwrap();
        let result = decrypt_payload(
            &encrypted, &key, "rec:other", &DataTier::Critical
        );
    ...
    fn aad_tier_mismatch_fails_decryption() {
        ...
        let original = b"sovereign data";
        let encrypted = encrypt_payload(
            original, &key, "rec:tier-test", &DataTier::Critical
        ).unwrap();
        let result = decrypt_payload(
            &encrypted, &key, "rec:tier-test", &DataTier::Personal
        );
        assert_eq!(result, Err(EdisonError::DecryptionFailed));
    }
    ...
    fn store_saves_and_loads() {
        ...
        let mut store = Store::new();
        let r = Record::new("rec:40", DataTier::Personal,
            "owner_abc", vec![1,2,3], [0u8; 32]).unwrap();
        store.write(r).unwrap();
    ...
    fn audit_log_persists() {
        ...
        let _ = std::fs::remove_file(path);
        let mut store = Store::new();
        let r = Record::new("rec:50", DataTier::Personal,
            "owner_abc", vec![1], [0u8; 32]).unwrap();
        store.write(r).unwrap();
    ...
    fn duplicate_id_rejected() {
        let mut store = Store::new();
        let r1 = Record::new("rec:100", DataTier::Personal,
            "alice", vec![1], [0u8; 32]).unwrap();
        let r2 = Record::new("rec:100", DataTier::Personal,
