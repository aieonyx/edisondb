...
//   [32] audit_hash: [u8;32] (last prev_hash from audit chain)
//   [32] seal      : [u8;32] (SHA-256 of all above fields)
// Total: 78 bytes

use crate::{DataTier, AuditEntry};
...
impl ArpiHeader {
    ...
    pub fn from_bytes(b: &[u8; HEADER_SIZE]) -> Option<Self> {
        let version = b[0];
        if version != ARPI_VERSION { return None; }
        ...
        Some(Self { version, tier, timestamp, count, audit_hash, seal })
    }

    /// Hex string of audit_hash for logging / display.
    pub fn audit_hash_hex(&self) -> String {
        self.audit_hash.iter().map(|b| format!("{:02x}", b)).collect()
    }
...
/// Extract the last prev_hash from an audit log, or zeros if empty.
pub fn last_audit_hash(entries: &[AuditEntry]) -> [u8; 32] {
    entries.last().map(|e| e.prev_hash).unwrap_or([0u8; 32])
}
