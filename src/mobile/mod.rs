...
/// ARPi header layout — 78 bytes, fixed.
...
/// 56      22    node_id (UTF-8, zero-padded, device ARPi ID)
#[derive(Debug, Clone)]
pub struct ArpiHeader {
    pub magic: [u8; 4],        // b"ARPi"
...
/// Stored value layout: [arpi_header: 78 bytes][value_bytes: variable]
...
impl MobileDb {
    ...
    pub fn open(path: &str) -> Result<Self, DbError> {
        let p = Path::new(path);
        let keyspace = Config::new(p)
            .open()
            .map_err(DbError::Fjall)?;
        let partition = keyspace
            .open_partition("main", fjall::PartitionCreateOptions::default())
            .map_err(DbError::Fjall)?;

        // Recover monotonic counter from a dedicated key.
    ...
    /// Insert a key/value pair with the supplied ARPi provenance header.
    /// The header is validated (magic + BLAKE3 verification) before write.
    pub fn insert(&mut self, key: &str, value: &str, arpi_raw: &[u8]) -> Result<(), DbError> {
        ...
        // Verify BLAKE3 hash in header matches value bytes.
        ...
        // Rebuild header with correct monotonic counter before storing.
    ...
    /// Retrieve the value for `key`, returning None if absent.
    pub fn query(&self, key: &str) -> Result<Option<String>, DbError> {
        match self.partition.get(key.as_bytes()) {
            Ok(Some(record)) => {
                ...
                let s = String::from_utf8_lossy(value_bytes).into_owned();
                Ok(Some(s))
            }
            Ok(None) => Ok(None),
